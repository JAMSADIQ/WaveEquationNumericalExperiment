...
def solver(I, V, f, c, L, dt, C, T, xmin=0,   user_action=None):
    ...
    dudt = np.zeros(Nx+1, dtype=np.float64)
    Dudt = np.zeros(Nx+1, dtype=np.float64)
    Dudr = np.zeros(Nx+1, dtype=np.float64)
    ...
    #For L2 norm errors
    #Err_t = spatial_Sq_sum_error(u_n, x, t[n+1])
    #Errt_array[0] = Err_t
    ...
    for n in range(1, int(Nt)):
        ...
        for i in range(1, Nx):
            d0[i] = 2.0*((u_n[i+1] + u_n[i-1]) - 2.0*(1.0 - 2.0/C2)*u_n[i]) \
                    ...
                    +4.0*dt**2/(dx*((i+0.5)*dx)*C2)*(u_n[i+1] - u_n[i-1]) \
        ...
        d0[Nx] = 4.0*dt**2/C2*f(x[Nx], t[n]) - dx/dt*u_nm3[Nx] \
                + (2.0 - 8.0/(2.0*Nx +1.0))*dx/dt* u_nm2[Nx] \
                + 2.0*u_nm1[Nx-1] - 2.0*u_nm1[Nx] - 4.0/C2*u_nm1[Nx]\
                + dx/dt*(4.0 + 32.0/(2.0*Nx + 1.0))*u_nm1[Nx] \
                + 4.0*u_n[Nx-1] - 4.0*u_n[Nx] \
                + 8.0/C2*u_n[Nx] - dx/dt*(2.0 + 24.0/(2.0*Nx + 1.0))*u_n[Nx]
